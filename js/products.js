/**
 * HÖRBI products — images under media/catalog/
 * Generated content from brand docs + photo folders.
 */
window.HORBI_PRODUCTS = [
  {
    id: "myata",
    status: "live",
    flavorKey: "mint",
    sku: "CHL-MINT-500",
    images: [
      "media/web/myata/00.jpg",
      "media/web/myata/01.jpg",
      "media/web/myata/02.jpg",
      "media/web/myata/03.jpg",
      "media/web/myata/04.jpg",
      "media/web/myata/05.jpg",
      "media/web/myata/06.jpg",
      "media/web/myata/07.jpg",
      "media/web/myata/08.jpg",
      "media/web/myata/09.jpg",
      "media/web/myata/10.jpg",
      "media/web/myata/11.jpg",
      "media/web/myata/12.jpg",
      "media/web/myata/13.jpg",
      "media/web/myata/14.jpg",
      "media/web/myata/15.jpg",
      "media/web/myata/16.jpg",
      "media/web/myata/17.jpg",
      "media/web/myata/18.jpg",
      "media/web/myata/19.jpg",
      "media/web/myata/20.jpg",
      "media/web/myata/21.jpg",
      "media/web/myata/22.jpg",
      "media/web/myata/23.jpg",
      "media/web/myata/24.jpg",
      "media/web/myata/25.jpg",
      "media/web/myata/26.jpg",
      "media/web/myata/27.jpg",
      "media/web/myata/28.jpg",
      "media/web/myata/29.jpg",
      "media/web/myata/30.jpg"
    ],
    wb: "https://www.wildberries.ru/catalog/1094870954/detail.aspx",
    i18n: {
      ru: {
        name: "HÖRBI Хлорофилл «Дикая мята» 500 мл",
        short: "Освежающий вкус мяты · до 100 порций",
        descriptionHtml: `
          <p class="lead">Жидкий хлорофилл-концентрат HÖRBI со вкусом дикой мяты — источник хлорофилла и дополнительный источник меди. Освежающий вкус делает ежедневный приём комфортным.</p>
          <p class="note">Не является лекарственным средством.</p>
          <h4>Преимущества</h4>
          <ul>
            <li>Источник хлорофилла</li>
            <li>Дополнительный источник меди</li>
            <li>Освежающий вкус мяты</li>
            <li>Жидкая форма, удобно разводить в воде или соке</li>
            <li>Объём 500 мл · до 100 порций</li>
            <li>Импортное сырьё высокого качества</li>
          </ul>
          <h4>Активные компоненты</h4>
          <p>Суточная дозировка 5–15 мл:</p>
          <table class="spec">
            <thead><tr><th>Компонент</th><th>Содержание</th></tr></thead>
            <tbody>
              <tr><td>Хлорофилл</td><td>15–45 мг</td></tr>
              <tr><td>Медь</td><td>0,75–2,25 мг (75–225% нормы)</td></tr>
            </tbody>
          </table>
          <h4>Как принимать</h4>
          <ul>
            <li>Перед употреблением взболтать</li>
            <li>Взрослым: 1 ч. л. (5 мл) 1–3 раза в день во время еды</li>
            <li>Развести в ½ стакана воды или сока</li>
            <li>Курс — 1 месяц; при необходимости повторить</li>
            <li>Перед применением рекомендуется консультация врача</li>
          </ul>
          <h4>Состав</h4>
          <p>Вода очищенная, глицерин, натуральный ароматизатор мята, сорбит, хлорофилл (медный комплекс хлорофиллина натрия), консерванты (сорбат калия, бензоат натрия). Содержит подсластитель сорбит.</p>
          <h4>Важно</h4>
          <ul>
            <li>Срок годности: 2 года</li>
            <li>Хранить до +25 °C, без прямого солнца</li>
            <li>После вскрытия — в холодильнике</li>
            <li>Противопоказания: индивидуальная непереносимость, беременность, кормление грудью</li>
          </ul>
        `,
      },
      en: {
        name: "HÖRBI Chlorophyll “Wild Mint” 500 ml",
        short: "Refreshing mint taste · up to 100 servings",
        descriptionHtml: `
          <p class="lead">Liquid chlorophyll concentrate with wild-mint taste — a source of chlorophyll and an additional source of copper. A fresh mint profile makes daily use easy.</p>
          <p class="note">Not a medicinal product.</p>
          <h4>Key benefits</h4>
          <ul>
            <li>Source of chlorophyll</li>
            <li>Additional copper</li>
            <li>Refreshing mint taste</li>
            <li>Liquid format — mix with water or juice</li>
            <li>500 ml · up to 100 servings</li>
          </ul>
          <h4>Active compounds (5–15 ml / day)</h4>
          <table class="spec">
            <thead><tr><th>Component</th><th>Amount</th></tr></thead>
            <tbody>
              <tr><td>Chlorophyll</td><td>15–45 mg</td></tr>
              <tr><td>Copper</td><td>0.75–2.25 mg</td></tr>
            </tbody>
          </table>
          <h4>How to use</h4>
          <ul>
            <li>Shake before use</li>
            <li>Adults: 1 tsp (5 ml), 1–3 times daily with meals</li>
            <li>Dilute in ½ glass of water or juice</li>
            <li>Course: 1 month</li>
          </ul>
        `,
      },
      de: {
        name: "HÖRBI Chlorophyll „Wilde Minze“ 500 ml",
        short: "Frischer Minzgeschmack · bis 100 Portionen",
        descriptionHtml: `
          <p class="lead">Flüssiges Chlorophyll-Konzentrat mit Minzgeschmack — Quelle von Chlorophyll und zusätzlicher Kupferquelle. Erfrischender Geschmack für den Alltag.</p>
          <p class="note">Kein Arzneimittel.</p>
          <h4>Vorteile</h4>
          <ul>
            <li>Chlorophyll-Quelle</li>
            <li>Zusätzliches Kupfer</li>
            <li>Erfrischender Minzgeschmack</li>
            <li>500 ml · bis 100 Portionen</li>
          </ul>
          <h4>Wirkstoffe (5–15 ml / Tag)</h4>
          <table class="spec">
            <thead><tr><th>Komponente</th><th>Gehalt</th></tr></thead>
            <tbody>
              <tr><td>Chlorophyll</td><td>15–45 mg</td></tr>
              <tr><td>Kupfer</td><td>0,75–2,25 mg</td></tr>
            </tbody>
          </table>
          <h4>Anwendung</h4>
          <ul>
            <li>Vor Gebrauch schütteln</li>
            <li>Erwachsene: 1 TL (5 ml), 1–3× täglich zu den Mahlzeiten</li>
            <li>In ½ Glas Wasser oder Saft verdünnen</li>
          </ul>
        `,
      },
      it: {
        name: "HÖRBI Clorofilla “Menta selvatica” 500 ml",
        short: "Gusto menta · fino a 100 porzioni",
        descriptionHtml: `
          <p class="lead">Concentrato di clorofilla liquida al gusto menta — fonte di clorofilla e fonte aggiuntiva di rame. Gusto fresco per l’uso quotidiano.</p>
          <p class="note">Non è un medicinale.</p>
          <h4>Vantaggi</h4>
          <ul>
            <li>Fonte di clorofilla</li>
            <li>Rame aggiuntivo</li>
            <li>Gusto menta rinfrescante</li>
            <li>500 ml · fino a 100 porzioni</li>
          </ul>
          <h4>Componenti attivi (5–15 ml / giorno)</h4>
          <table class="spec">
            <thead><tr><th>Componente</th><th>Quantità</th></tr></thead>
            <tbody>
              <tr><td>Clorofilla</td><td>15–45 mg</td></tr>
              <tr><td>Rame</td><td>0,75–2,25 mg</td></tr>
            </tbody>
          </table>
          <h4>Modalità d’uso</h4>
          <ul>
            <li>Agitare prima dell’uso</li>
            <li>Adulti: 1 cucchiaino (5 ml), 1–3 volte al giorno con i pasti</li>
            <li>Diluire in ½ bicchiere d’acqua o succo</li>
          </ul>
        `,
      },
    },
  },
  {
    id: "smorodina",
    status: "live",
    flavorKey: "blackcurrant",
    sku: "CHL-CURR-500",
    images: [
      "media/web/smorodina/00.jpg",
      "media/web/smorodina/01.jpg",
      "media/web/smorodina/02.jpg",
      "media/web/smorodina/03.jpg",
      "media/web/smorodina/04.jpg",
      "media/web/smorodina/05.jpg",
      "media/web/smorodina/06.jpg",
      "media/web/smorodina/07.jpg",
      "media/web/smorodina/08.jpg",
      "media/web/smorodina/09.jpg",
      "media/web/smorodina/10.jpg",
      "media/web/smorodina/11.jpg",
      "media/web/smorodina/12.jpg",
      "media/web/smorodina/13.jpg",
      "media/web/smorodina/14.jpg",
      "media/web/smorodina/15.jpg",
      "media/web/smorodina/16.jpg",
      "media/web/smorodina/17.jpg",
      "media/web/smorodina/18.jpg",
      "media/web/smorodina/19.jpg",
      "media/web/smorodina/20.jpg",
      "media/web/smorodina/21.jpg",
      "media/web/smorodina/22.jpg",
      "media/web/smorodina/23.jpg",
      "media/web/smorodina/24.jpg",
      "media/web/smorodina/25.jpg",
      "media/web/smorodina/26.jpg",
      "media/web/smorodina/27.jpg",
      "media/web/smorodina/28.jpg"
    ],
    wb: "https://www.wildberries.ru/catalog/1095345290/detail.aspx",
    i18n: {
      ru: {
        name: "HÖRBI Хлорофилл «Сочная смородина» 500 мл",
        short: "Ягодный вкус · жидкая форма",
        descriptionHtml: `
          <p class="lead">Жидкий хлорофилл HÖRBI со вкусом сочной чёрной смородины — источник хлорофилла и дополнительный источник меди на основе медного комплекса хлорофиллина натрия из люцерны.</p>
          <p class="note">Не является лекарственным средством.</p>
          <h4>Преимущества</h4>
          <ul>
            <li>Источник хлорофилла</li>
            <li>Дополнительный источник меди</li>
            <li>Яркий ягодный вкус</li>
            <li>Импортное сырьё · натуральные ароматизаторы</li>
            <li>Объём 500 мл · удобное применение</li>
          </ul>
          <h4>Активные компоненты</h4>
          <p>Суточная порция 20–40 мл:</p>
          <table class="spec">
            <thead><tr><th>Компонент</th><th>Содержание</th></tr></thead>
            <tbody>
              <tr><td>Хлорофилл</td><td>25–50 мг</td></tr>
              <tr><td>Медь</td><td>1,3–2,6 мг (130–260% АУП)</td></tr>
            </tbody>
          </table>
          <h4>Как принимать</h4>
          <ul>
            <li>Перед употреблением взболтать</li>
            <li>Взрослым: 2 ст. л. (20 мл) 1–2 раза в день во время еды</li>
            <li>В чистом виде или развести в ½ стакана воды</li>
            <li>Курс — 1 месяц</li>
          </ul>
          <h4>Состав</h4>
          <p>Вода очищенная структурированная, хлорофилл (медный комплекс хлорофиллина натрия из люцерны), яблочная кислота, ксантановая камедь, консерванты (сорбат калия, бензоат натрия), ароматизаторы натуральные (мята, чёрная смородина), стевиогликозиды.</p>
          <h4>Важно</h4>
          <ul>
            <li>После вскрытия хранить в холодильнике</li>
            <li>Противопоказания: индивидуальная непереносимость, беременность, ГВ</li>
          </ul>
        `,
      },
      en: {
        name: "HÖRBI Chlorophyll “Juicy Blackcurrant” 500 ml",
        short: "Berry taste · liquid concentrate",
        descriptionHtml: `
          <p class="lead">Liquid chlorophyll with juicy blackcurrant taste — chlorophyll source plus additional copper, based on sodium copper chlorophyllin from alfalfa.</p>
          <p class="note">Not a medicinal product.</p>
          <h4>Benefits</h4>
          <ul>
            <li>Chlorophyll source</li>
            <li>Additional copper</li>
            <li>Bright berry taste</li>
            <li>500 ml convenient liquid format</li>
          </ul>
          <h4>Active compounds (20–40 ml / day)</h4>
          <table class="spec">
            <thead><tr><th>Component</th><th>Amount</th></tr></thead>
            <tbody>
              <tr><td>Chlorophyll</td><td>25–50 mg</td></tr>
              <tr><td>Copper</td><td>1.3–2.6 mg</td></tr>
            </tbody>
          </table>
          <h4>How to use</h4>
          <ul>
            <li>Shake before use</li>
            <li>Adults: 2 tbsp (20 ml), 1–2 times daily with meals</li>
            <li>Straight or diluted in ½ glass of water</li>
          </ul>
        `,
      },
      de: {
        name: "HÖRBI Chlorophyll „Saftige Johannisbeere“ 500 ml",
        short: "Beerig im Geschmack · flüssig",
        descriptionHtml: `
          <p class="lead">Flüssiges Chlorophyll mit Johannisbeergeschmack — Chlorophyllquelle und zusätzliche Kupferquelle.</p>
          <p class="note">Kein Arzneimittel.</p>
          <h4>Vorteile</h4>
          <ul>
            <li>Chlorophyll-Quelle</li>
            <li>Zusätzliches Kupfer</li>
            <li>Fruchtig-beeriger Geschmack</li>
            <li>500 ml Flüssigkonzentrat</li>
          </ul>
          <h4>Wirkstoffe (20–40 ml / Tag)</h4>
          <table class="spec">
            <thead><tr><th>Komponente</th><th>Gehalt</th></tr></thead>
            <tbody>
              <tr><td>Chlorophyll</td><td>25–50 mg</td></tr>
              <tr><td>Kupfer</td><td>1,3–2,6 mg</td></tr>
            </tbody>
          </table>
        `,
      },
      it: {
        name: "HÖRBI Clorofilla “Ribes nero succoso” 500 ml",
        short: "Gusto di frutti di bosco · liquido",
        descriptionHtml: `
          <p class="lead">Clorofilla liquida al gusto di ribes nero — fonte di clorofilla e fonte aggiuntiva di rame.</p>
          <p class="note">Non è un medicinale.</p>
          <h4>Vantaggi</h4>
          <ul>
            <li>Fonte di clorofilla</li>
            <li>Rame aggiuntivo</li>
            <li>Gusto fruttato intenso</li>
            <li>Formato liquido 500 ml</li>
          </ul>
          <h4>Componenti attivi (20–40 ml / giorno)</h4>
          <table class="spec">
            <thead><tr><th>Componente</th><th>Quantità</th></tr></thead>
            <tbody>
              <tr><td>Clorofilla</td><td>25–50 mg</td></tr>
              <tr><td>Rame</td><td>1,3–2,6 mg</td></tr>
            </tbody>
          </table>
        `,
      },
    },
  },
  {
    id: "marakuya",
    status: "live",
    flavorKey: "passionfruit",
    sku: "CHL-PASS-500",
    images: [
      "media/web/marakuya/00.jpg",
      "media/web/marakuya/01.jpg",
      "media/web/marakuya/02.jpg",
      "media/web/marakuya/03.jpg",
      "media/web/marakuya/04.jpg",
      "media/web/marakuya/05.jpg",
      "media/web/marakuya/06.jpg",
      "media/web/marakuya/07.jpg",
      "media/web/marakuya/08.jpg",
      "media/web/marakuya/09.jpg",
      "media/web/marakuya/10.jpg",
      "media/web/marakuya/11.jpg",
      "media/web/marakuya/12.jpg",
      "media/web/marakuya/13.jpg",
      "media/web/marakuya/14.jpg",
      "media/web/marakuya/15.jpg",
      "media/web/marakuya/16.jpg",
      "media/web/marakuya/17.jpg",
      "media/web/marakuya/18.jpg",
      "media/web/marakuya/19.jpg",
      "media/web/marakuya/20.jpg",
      "media/web/marakuya/21.jpg",
      "media/web/marakuya/22.jpg",
      "media/web/marakuya/23.jpg",
      "media/web/marakuya/24.jpg",
      "media/web/marakuya/25.jpg",
      "media/web/marakuya/26.jpg",
      "media/web/marakuya/27.jpg",
      "media/web/marakuya/28.jpg",
      "media/web/marakuya/29.jpg",
      "media/web/marakuya/30.jpg",
      "media/web/marakuya/31.jpg",
      "media/web/marakuya/32.jpg"
    ],
    wb: "https://www.wildberries.ru/catalog/1095385962/detail.aspx",
    i18n: {
      ru: {
        name: "HÖRBI Хлорофилл «Тропическая маракуйя» 500 мл",
        short: "Экзотический вкус · 500 мл",
        descriptionHtml: `
          <p class="lead">Жидкий хлорофилл HÖRBI со вкусом тропической маракуйи — источник хлорофилла и дополнительный источник меди с ярким фруктовым профилем.</p>
          <p class="note">Не является лекарственным средством.</p>
          <h4>Преимущества</h4>
          <ul>
            <li>Источник хлорофилла</li>
            <li>Дополнительный источник меди</li>
            <li>Экзотический вкус маракуйи</li>
            <li>Импортное сырьё · натуральные ароматизаторы</li>
            <li>Большой объём 500 мл</li>
          </ul>
          <h4>Активные компоненты</h4>
          <p>Суточная порция 20–40 мл:</p>
          <table class="spec">
            <thead><tr><th>Компонент</th><th>Содержание</th></tr></thead>
            <tbody>
              <tr><td>Хлорофилл</td><td>25–50 мг</td></tr>
              <tr><td>Медь</td><td>1,3–2,6 мг</td></tr>
            </tbody>
          </table>
          <h4>Как принимать</h4>
          <ul>
            <li>Перед употреблением взболтать</li>
            <li>Взрослым: 2 ст. л. (20 мл) 1–2 раза в день во время еды</li>
            <li>В чистом виде или развести водой</li>
            <li>Курс — 1 месяц</li>
          </ul>
          <h4>Состав</h4>
          <p>Вода очищенная структурированная, хлорофилл (медный комплекс хлорофиллина натрия из люцерны), яблочная кислота, ксантановая камедь, консерванты (сорбат калия, бензоат натрия), ароматизаторы натуральные (мята, маракуйя), стевиогликозиды.</p>
          <h4>Важно</h4>
          <ul>
            <li>Хранить до +25 °C; после вскрытия — в холодильнике</li>
            <li>Противопоказания: индивидуальная непереносимость, беременность, ГВ</li>
          </ul>
        `,
      },
      en: {
        name: "HÖRBI Chlorophyll “Tropical Passion Fruit” 500 ml",
        short: "Exotic taste · 500 ml",
        descriptionHtml: `
          <p class="lead">Liquid chlorophyll with tropical passion-fruit taste — chlorophyll source and additional copper in a bright fruity profile.</p>
          <p class="note">Not a medicinal product.</p>
          <h4>Benefits</h4>
          <ul>
            <li>Chlorophyll source</li>
            <li>Additional copper</li>
            <li>Exotic passion-fruit taste</li>
            <li>500 ml value format</li>
          </ul>
          <h4>Active compounds (20–40 ml / day)</h4>
          <table class="spec">
            <thead><tr><th>Component</th><th>Amount</th></tr></thead>
            <tbody>
              <tr><td>Chlorophyll</td><td>25–50 mg</td></tr>
              <tr><td>Copper</td><td>1.3–2.6 mg</td></tr>
            </tbody>
          </table>
        `,
      },
      de: {
        name: "HÖRBI Chlorophyll „Tropische Maracuja“ 500 ml",
        short: "Exotischer Geschmack · 500 ml",
        descriptionHtml: `
          <p class="lead">Flüssiges Chlorophyll mit tropischem Maracuja-Geschmack — Chlorophyllquelle und zusätzliche Kupferquelle.</p>
          <p class="note">Kein Arzneimittel.</p>
          <h4>Vorteile</h4>
          <ul>
            <li>Chlorophyll-Quelle</li>
            <li>Zusätzliches Kupfer</li>
            <li>Exotischer Maracuja-Geschmack</li>
            <li>500 ml Format</li>
          </ul>
          <h4>Wirkstoffe (20–40 ml / Tag)</h4>
          <table class="spec">
            <thead><tr><th>Komponente</th><th>Gehalt</th></tr></thead>
            <tbody>
              <tr><td>Chlorophyll</td><td>25–50 mg</td></tr>
              <tr><td>Kupfer</td><td>1,3–2,6 mg</td></tr>
            </tbody>
          </table>
        `,
      },
      it: {
        name: "HÖRBI Clorofilla “Maracuja tropicale” 500 ml",
        short: "Gusto esotico · 500 ml",
        descriptionHtml: `
          <p class="lead">Clorofilla liquida al gusto di maracuja — fonte di clorofilla e fonte aggiuntiva di rame.</p>
          <p class="note">Non è un medicinale.</p>
          <h4>Vantaggi</h4>
          <ul>
            <li>Fonte di clorofilla</li>
            <li>Rame aggiuntivo</li>
            <li>Gusto esotico di maracuja</li>
            <li>Formato 500 ml</li>
          </ul>
          <h4>Componenti attivi (20–40 ml / giorno)</h4>
          <table class="spec">
            <thead><tr><th>Componente</th><th>Quantità</th></tr></thead>
            <tbody>
              <tr><td>Clorofilla</td><td>25–50 mg</td></tr>
              <tr><td>Rame</td><td>1,3–2,6 mg</td></tr>
            </tbody>
          </table>
        `,
      },
    },
  },
  // Coming soon
  {
    id: "soon-testobooster",
    status: "soon",
    images: [],
    wb: "",
    i18n: {
      ru: {
        name: "Тестобустер",
        short: "Капсулы · скоро в линейке HÖRBI",
        descriptionHtml: `<p class="lead">БАД «Тестобустер» — в подготовке к запуску.</p><p class="soon-line">Скоро</p><p>Форма: капсулы. Комплекс на основе D-аспарагиновой кислоты, экстрактов маки и пажитника, цинка.</p>`,
      },
      en: {
        name: "Testobooster",
        short: "Capsules · coming to HÖRBI",
        descriptionHtml: `<p class="lead">“Testobooster” food supplement is in preparation.</p><p class="soon-line">Coming soon</p>`,
      },
      de: {
        name: "Testobooster",
        short: "Kapseln · bald bei HÖRBI",
        descriptionHtml: `<p class="lead">Nahrungsergänzung „Testobooster“ in Vorbereitung.</p><p class="soon-line">Demnächst</p>`,
      },
      it: {
        name: "Testobooster",
        short: "Capsule · in arrivo",
        descriptionHtml: `<p class="lead">Integratore “Testobooster” in preparazione.</p><p class="soon-line">Presto</p>`,
      },
    },
  },
  {
    id: "soon-magnesium-citrate",
    status: "soon",
    images: [],
    wb: "",
    i18n: {
      ru: {
        name: "Цитрат магния + B6",
        short: "Магний 480 мг · витамин B6",
        descriptionHtml: `<p class="lead">БАД «Цитрат магния 800 мг + B6» — скоро в каталоге.</p><p class="soon-line">Скоро</p><p>Дополнительный источник магния и витамина B6 в капсулах.</p>`,
      },
      en: {
        name: "Magnesium citrate + B6",
        short: "Magnesium · vitamin B6",
        descriptionHtml: `<p class="lead">Magnesium citrate + B6 — coming soon.</p><p class="soon-line">Coming soon</p>`,
      },
      de: {
        name: "Magnesiumcitrat + B6",
        short: "Magnesium · Vitamin B6",
        descriptionHtml: `<p class="lead">Magnesiumcitrat + B6 — demnächst.</p><p class="soon-line">Demnächst</p>`,
      },
      it: {
        name: "Citrato di magnesio + B6",
        short: "Magnesio · vitamina B6",
        descriptionHtml: `<p class="lead">Citrato di magnesio + B6 — in arrivo.</p><p class="soon-line">Presto</p>`,
      },
    },
  },
  {
    id: "soon-inositol",
    status: "soon",
    images: [],
    wb: "",
    i18n: {
      ru: {
        name: "Инозитол + фолиевая кислота",
        short: "Инозитол 1000 мг · B9",
        descriptionHtml: `<p class="lead">Комплекс инозитол + фолиевая кислота — в подготовке.</p><p class="soon-line">Скоро</p>`,
      },
      en: {
        name: "Inositol + folic acid",
        short: "Inositol · vitamin B9",
        descriptionHtml: `<p class="lead">Inositol + folic acid complex — coming soon.</p><p class="soon-line">Coming soon</p>`,
      },
      de: {
        name: "Inositol + Folsäure",
        short: "Inositol · Vitamin B9",
        descriptionHtml: `<p class="lead">Inositol + Folsäure — demnächst.</p><p class="soon-line">Demnächst</p>`,
      },
      it: {
        name: "Inositolo + acido folico",
        short: "Inositolo · vitamina B9",
        descriptionHtml: `<p class="lead">Inositolo + acido folico — in arrivo.</p><p class="soon-line">Presto</p>`,
      },
    },
  },
  {
    id: "soon-magnesium-chelate",
    status: "soon",
    images: [],
    wb: "",
    i18n: {
      ru: {
        name: "Хелат магния + B6",
        short: "Магний 402 мг · B6",
        descriptionHtml: `<p class="lead">БАД «Хелат магния + B6» — скоро.</p><p class="soon-line">Скоро</p>`,
      },
      en: {
        name: "Magnesium chelate + B6",
        short: "Chelated magnesium · B6",
        descriptionHtml: `<p class="lead">Magnesium chelate + B6 — coming soon.</p><p class="soon-line">Coming soon</p>`,
      },
      de: {
        name: "Magnesium-Chelat + B6",
        short: "Chelatiertes Magnesium · B6",
        descriptionHtml: `<p class="lead">Magnesium-Chelat + B6 — demnächst.</p><p class="soon-line">Demnächst</p>`,
      },
      it: {
        name: "Chelato di magnesio + B6",
        short: "Magnesio chelato · B6",
        descriptionHtml: `<p class="lead">Chelato di magnesio + B6 — in arrivo.</p><p class="soon-line">Presto</p>`,
      },
    },
  },
  {
    id: "soon-collagen",
    status: "soon",
    images: [],
    wb: "",
    i18n: {
      ru: {
        name: "Морской коллаген + витамин C",
        short: "Коллаген · гиалуроновая кислота · C",
        descriptionHtml: `<p class="lead">Морской коллаген с гиалуроновой кислотой и витамином C — в подготовке.</p><p class="soon-line">Скоро</p>`,
      },
      en: {
        name: "Marine collagen + vitamin C",
        short: "Collagen · hyaluronic acid · C",
        descriptionHtml: `<p class="lead">Marine collagen with hyaluronic acid & vitamin C — coming soon.</p><p class="soon-line">Coming soon</p>`,
      },
      de: {
        name: "Meereskollagen + Vitamin C",
        short: "Kollagen · Hyaluron · C",
        descriptionHtml: `<p class="lead">Meereskollagen mit Hyaluronsäure & Vitamin C — demnächst.</p><p class="soon-line">Demnächst</p>`,
      },
      it: {
        name: "Collagene marino + vitamina C",
        short: "Collagene · acido ialuronico · C",
        descriptionHtml: `<p class="lead">Collagene marino con acido ialuronico e vitamina C — in arrivo.</p><p class="soon-line">Presto</p>`,
      },
    },
  },
];
